...
describe 'GET /db/classroom/:handle/member-sessions', ->
  ...
  beforeEach utils.wrap (done) ->
    ...
    @session1B = yield utils.makeLevelSession({codeLanguage: 'javascript', state: { complete: false }}, {creator: @student1, level: @levelB})
    @session2A = yield utils.makeLevelSession({codeLanguage: 'javascript', state: { complete: true }}, {creator: @student2, level: @levelA})
    @session2B = yield utils.makeLevelSession({codeLanguage: 'javascript', state: { complete: false }}, {creator: @student2, level: @levelB})
    yield utils.loginUser(@teacher)
